...
# 文档一级标题：行首一个 # 加空格
#
# 【必须带 re.MULTILINE，否则这行代码等于没写】
# 下面 load_chunks 里是用 H1_RE.search(整篇文本) 去找标题的。
# 不带 MULTILINE 时，^ 只认【整个字符串】的开头、$ 只认结尾，
# 而标题虽然在文件第一行、文件却远不止一行 ——
# 于是 search 永远匹配不上，doc_title 悄悄退回了文件名。
#
# 这个 bug 藏了很久：_build 里那行 DOC_TITLE_WEIGHT（给文档标题加权）
# 因此一直是死代码，白白浪费了一路本该有用的信号。
...
# ---- 组中文 bigram 时，含这些字的组合一律丢掉 ----
#
# 【先说清楚：这是一条工程启发式，不是中文分词的普遍规律】
# 「的」最常当结构助词用，作用是把前后两个词连起来。
# 在【本项目这个小语料】里，含「的」的两字组合基本都横跨词边界，是噪声：
#
#     "现在完成时的句子结构"
#        → "时的"  横跨「现在完成时」和「句子结构」两个词
#        → "的句"  横跨「的」和「句子」
#
# 【但它会误伤合法词 —— 这一点必须诚实写出来】
# 中文里确实有「目的」「的确」「的士」「的哥」这类「的」打头的词。
# 一刀切会把它们也切碎，导致这些词的召回变差。
#
# 本项目当前的 3 份资料和 33 道验收题都不含这类词，所以这个代价暂时是零；
# 但换成真实语料后必须重新评估。
# 把它写成可调常量、而不是把规则写死在逻辑里，就是为了到时候只改这一个字符串。
#
# 【为什么当前值得付这个代价】
# 有人问「现在完成时的句子结构是怎样的？」，正确答案在「基本结构」那一节。
# 但 "时的" 恰好命中了另一节的标题「和一般过去时的区别」，
# 而标题里的词要加权 3 倍，于是一个跨词边界的碎片把真正该被检索到的
# 「基本结构」挤出了前三名。丢掉含「的」的组合之后，「基本结构」回到第一名。
#
# 【通用性】它针对的是「一类字」，不针对任何题目、ID 或文件名。
BIGRAM_STOP_CHARS = "的"
...
def tokenize(text):
    ...
    for run in CJK_RUN_RE.findall(text):                    # 再处理中文片段
        if len(run) == 1:                                   # 只有一个字，没法组 bigram，就单独算一个词
            if run not in BIGRAM_STOP_CHARS:                # 但如果它是结构助词本身（比如孤零零一个「的」），丢掉
                tokens.append(run)
        else:
            for i in range(len(run) - 1):                   # 挨着的两个字组成一个词
                pair = run[i:i + 2]
                # 【关键】含这些字的组合一律跳过，理由和代价见上面 BIGRAM_STOP_CHARS 的说明。
                # 它们【通常】横跨词边界、不是词，只会给别的片段白送分——
                # 但「目的」「的确」这类合法词会被误伤，这是已知取舍。
                if pair[0] in BIGRAM_STOP_CHARS or pair[1] in BIGRAM_STOP_CHARS:
                    continue
                tokens.append(pair)
